...
export const ExchangePage = () => {
  ...
  const handleExchange = async () => {
    setIsLoading(true);
    try {
      await ExhangeTokens(publicKey, toAddress, fromToken, toToken, amount);
      await getBalances();
      toast.success("Exchanged succesfully")
      setAmount("")
      setFromToken("")
      setToToken("")
      setToAddress("")
  ...
  return (
    <div className="bg-gray-900 text-white min-h-screen flex flex-col items-center justify-center">
      ...
      <div className="bg-gray-800 rounded-lg shadow-md p-8 w-full max-w-md">
        <h1 className="text-2xl font-bold text-center mb-6">Exchange Tokens</h1>
        <form
          onSubmit={(e) => {
            e.preventDefault();
            handleExchange();
        ...
        >
          {/* From Token */}
          <div className="mb-4">
            <label className="block text-sm font-medium mb-2">Token you want to send</label>
            <select
              value={fromToken}
          ...
          {/* To Token */}
          <div className="mb-4">
            <label className="block text-sm font-medium mb-2">Token receiver should receive</label>
            <select
              value={toToken}
          ...
          {toToken !== "" && (
            <div className="mt-4">
              <p>Platform charges: 0.1 {toToken}</p>
            </div>
          )}
        </form>
      </div>
